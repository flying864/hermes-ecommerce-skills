...
metadata:
  hermes:
    ...
    related_skills: [subagent-driven-development, youtube-content]
---
...
- 当用户说“测试运行一下”时，仍按完整选品流程执行，并默认交付美观 HTML 报告附件，而不是只在聊天里给长文本摘要。
...
1. **YouTube Agent**
   - 搜索关键词及 pain point 关键词。
   - 分析视频标题、播放量、发布时间、频道类型、评论情绪、常见问题、购买动机。
   - 优先使用 transcript；没有字幕时根据标题、描述、评论和公开信息分析。
   - 如可用，使用 `youtube-content` skill 获取 transcript。
...
最终报告默认用中文。**默认输出为美观 HTML 文件附件；聊天里只发简短结论 + 附件，不要长篇纯文本。** 除非用户明确要求聊天内全文，否则不要把完整报告直接贴到聊天里。
