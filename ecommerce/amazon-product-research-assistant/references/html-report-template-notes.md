...
- **默认必须包含视觉资产**：只要是 Amazon 选品/产品分析 HTML 报告，就应包含 Amazon 竞品/产品图片和 Google Trends 近 1 年 + 近 5 年趋势图。除非明确无法获取，否则缺少图片或趋势图视为未完成。
...
- [ ] HTML 是美观卡片式布局，不是纯文本/表格堆砌。
- [ ] 聊天回复只包含简短结论 + 附件，不粘贴完整长报告。
- [ ] 报告包含 Amazon 产品/竞品图片；图片文件已下载到资源目录并在 HTML 中用相对路径引用。
- [ ] 每个产品/竞品图片下方有稳定 Amazon `/dp/{ASIN}` 可点击链接，并显示 ASIN、价格、评分、评论数。
- [ ] 报告包含 Google Trends 近 1 年和近 5 年趋势图；优先真实导出/浏览器数据，失败时才用明确标注的代理图。
- [ ] 如果 HTML 引用任何本地资源（图片、趋势图、CSS/JS），发送 zip，而不是只发送单个 HTML。
- [ ] 发送前验证：HTML 文件存在且大小合理；`<img>` 标签数量符合预期；所有 `src="..."` 相对资源都存在；zip 内包含 HTML 和资源目录。
- [ ] 底部“数据来源 / 消息来源”详细列明 Amazon、Google Trends、YouTube/TikTok、供应链、合规、第三方工具缺口、数据时间。
